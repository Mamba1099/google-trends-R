  ...
  output$plot <- renderPlot({
    ...
    if (input$plot_type == "Histogram / Bar Graph") {
      ...
      for (i in seq_along(plots)) {
        ...
        plots[[i]] <- ggplot(data = numeric_data, aes_string(x = x_var, y = y_var)) +
          geom_point() +
          geom_density_2d() +
          theme_minimal()
      }
...
output$scatter_warning <- renderText({
  if (input$plot_type == "Scatter Matrix" &&
      (input$data_type == "tv hurricane by network" || input$data_type == "tv hurricanes")) {
    "Scatter Matrix cannot be plotted for this data"
  }
  })
output$correlation_warning <- renderText({
